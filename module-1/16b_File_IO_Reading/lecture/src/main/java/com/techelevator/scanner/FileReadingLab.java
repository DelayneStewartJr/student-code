...
public class FileReadingLab {
    ...
    public void run() {
        ...
        while (true) {
            ...
            String path = ""; // 1. Update path with correct code; what tool can you use to capture user input from the keyboard

            // Validate the input file
            inputFile = new File(""); // 2. Update "" with correct code; What do you need to pass in the File() constructor initialize your inputFile object- hint:
            if (!inputFile.exists()) {
                System.out.println(" ? goes here ? " + " does not exist");
                continue;
            } else if (!inputFile.isFile()) {
                System.out.println(" ? goes here ? " + " is not a file");
                continue;
            }
            break;
        }
        ...
        try (Scanner inputScanner = new Scanner(inputFile.getAbsoluteFile())){
            while (true) { // 3. Replace true with correct code
                String line = ""; //4. Replace "" with correct code
                if (true) {   //5. Blank lines should be skipped - replace true with correct code
